...
const StocksListPage = () => {
  ...
  if (isError) return <p>There was an error loading the stocks</p>;

  if (!data) {
    return <p>There was an error loading the stocks</p>;
  }
    ...
    <div className="flex w-full md:p-4 h-auto">
      <div className="bg-white dark:bg-(--bg-primary_dark) lg:flex flex-col gap-1 w-full rounded-xl shadow-lg p-4 h-auto hidden">
        <TableGeneric
          ...
          rowPath="/stocks"
          addButton={true}
          addButtonPath="/stocks/create-new-stock"
          pageSize={10}
          addPagination={true}
          addPageSelector={true}
          tableHeading="Stocks Register"
        />
      </div>
      {/* // $ Mobile View */}
      <div className="grid lg:hidden gap-2 w-full p-2">
        ...
        />{" "}
        {data.length === 0 ? (
          <EmptyMobilePlaceholder message="No Stocks available yet" />
