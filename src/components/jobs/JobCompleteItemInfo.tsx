...
function Field({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="grid grid-cols-[130px_1fr] gap-2 items-start text-sm">
...
function CompletedJobDetails({ item }: Props) {
  ...
  return (
    ...
    >
      {/* ── LEFT: Image panel ── */}
      {/* Gallery */}
      <div className="flex-1 gap-2 min-h-0">
        <ImageGallery
          images={
            activeTab === "details"
              ? (item.images ?? [])
      ...
      <div className="flex flex-col gap-4 text-font dark:text-gray-100 rounded-md p-4 dark:border-gray-700/50 h-full">
        ...
        {activeTab === "action" && action && (
          <div className="flex flex-col gap-4">
            {/* Completion banner */}
            <div className="flex items-center gap-2 text-xs text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/40 rounded-md px-3 py-2">
