//$ This component display detailed information of the the maintenance request created and actioned data.

import Separator from "@/components/dashboardSidebar/Seperator";
import useGlobalContext from "@/context/useGlobalContext";
import { useById } from "@/utils/api";
import { Error } from "../features/Error";
import { PageLoadingSpinner } from "../features/PageLoadingSpinner";
import { toast } from "sonner";

// icons
import { X, Check } from "lucide-react";
import { Badge } from "../features/Badge";
import { badgeStyles } from "@/styles/badgeStyles";
import { sharedStyles } from "@/styles/shared";
import { cn } from "@/lib/utils";
import Avatar from "../header/Avatar";
import axios from "axios";
import { Spinner } from "../ui/spinner";
import { useApproveRequest } from "@/hooks/useApproveRequest";
import type { DisposalWorkflowResponse } from "@/schemas/disposalsSchemas";

function Field({ label, value }: { label: string; value?: string | null }) {
  if (!value) return null;
  return (
    <div className="grid grid-cols-[120px_1fr] gap-2 items-start text-sm">
      <span className="text-xs text-gray-500 dark:text-gray-400 pt-0.5">
        {label}
      </span>
      <span className="text-gray-900 dark:text-gray-100 capitalize font-medium">
        {value}
      </span>
    </div>
  );
}

type DisposalRequestApprovalProps = {
  // Index of the asset currently shown when a transfer bundles more than one item
  selectedAssetIndex: number;
  onSelectAsset: (index: number) => void;
};

function DisposalRequestApproval({
  selectedAssetIndex,
  onSelectAsset,
}: DisposalRequestApprovalProps) {
  const {
    selectedRowId,
    // showRejectRequestDialogGeneric,
    setShowRejectRequestDialogGeneric,
    setRejectConfig,
  } = useGlobalContext();

  /* -------------------------------------------------------------------------- */
  /*                          Item Data                                         */
  /* -------------------------------------------------------------------------- */

  const { data: item, isPending } = useById<DisposalWorkflowResponse>({
    id: selectedRowId ?? "",
    queryKey: ["disposals", "pending-approval-item"],
    resourcePath: "api/disposals",
    params: {
      status: "pending",
    },
  });

  const assets = item?.assets ?? [];
  const currentAsset = assets[selectedAssetIndex] ?? assets[0];

  /* -------------------------------------------------------------------------- */
  /*                          Submit Data Hook                                  */
  /* -------------------------------------------------------------------------- */

  const { submit: approveRequest, isPending: isApproving } = useApproveRequest({
    id: selectedRowId ?? "",
    resourcePath: "api/disposals",
    queryKey: ["disposals", "action: approve-item"],
    successMessage: `The Request for asset ${currentAsset?.assetID} was Successfully Approved!!!`,
    errorMessage: "Could not approve the asset disposal. Please try again.",
    redirectPath: "disposals/requests",
  });

  /* -------------------------------------------------------------------------- */
  /*                          Handle Approve                                    */
  /* -------------------------------------------------------------------------- */
  const handleApprove = async () => {
    try {
      await approveRequest({});
    } catch (error) {
      if (axios.isAxiosError<{ message: string }>(error)) {
        toast.error(error?.response?.data?.message);
      } else {
        toast.error("Failed to assign item");
      }
    }
  };

  if (isPending) {
    return <PageLoadingSpinner />;
  }

  // fallback UI if timeout reached
  if (!item || !currentAsset) {
    return <Error />;
  }

  /* -------------------------------------------------------------------------- */
  /*                          Handle Reject                                     */
  /* -------------------------------------------------------------------------- */

  const handleReject = () => {
    setRejectConfig({
      title: "Reject Disposal",
      message: "Are you sure you want to reject this disposal?",
      resourcePath: "api/disposals",
      redirectPath: "disposals/requests",
      queryKey: ["disposals", "action: reject"],
      successMessage: "The Request was Successfully Rejected.",
      errorMessage: "Could not reject the asset disposal. Please try again.",
    });
    setShowRejectRequestDialogGeneric(true);
  };

  return (
    <div className="flex flex-col gap-4 text-font dark:text-gray-100 rounded-md p-4 dark:border-gray-700/50 h-full">
      {/* // $ ── Header ── */}
      <div className="flex flex-col gap-2">
        <p className="text-[11px] uppercase tracking-widest text-gray-400 dark:text-gray-500 font-medium select-none">
          Disposal No ·{""}
        </p>
        <h1 className="text-lg md:text-xl font-semibold capitalize leading-tight">
          {currentAsset.equipment}
        </h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">
          Asset ID: {currentAsset.assetID}
        </p>
      </div>

      {/* ── Asset selector — only shown when a transfer bundles more than one item ── */}
      {assets.length > 1 && (
        <div className="flex flex-wrap gap-2">
          {assets.map((asset, index) => (
            <button
              key={asset.assetID}
              type="button"
              onClick={() => onSelectAsset(index)}
              className={cn(
                "rounded-md border p-1.5 text-xs font-medium capitalize transition-colors hover:cursor-pointer",
                index === selectedAssetIndex
                  ? "border-green-500 bg-green-400/10 text-green-400"
                  : "border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600",
              )}
            >
              {`Asset ${index + 1}`}
            </button>
          ))}
        </div>
      )}

      {/* ── Status badges ── */}
      <div className="flex flex-wrap gap-2 text-cxs">
        <Badge
          value={`${item.status ?? "medium"}`}
          styleMap={badgeStyles.families.transfer_status}
          className="capitalize"
        />
      </div>
      <Separator width="100%" className="mt-2 mb-4" />

      {/* ── Requester row ── */}
      {item.pending?.requestedBy && (
        <div className="flex items-center gap-3">
          <Avatar name={item.pending?.requestedBy} isFullName={true} />
          <div className="flex flex-col leading-snug">
            <span className="text-sm font-medium capitalize">
              {item?.pending.requestedBy}
            </span>
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {`Requested · ${item.disposalCreated}`}
            </span>
          </div>
        </div>
      )}

      {/* ── Structured fields ── */}
      <div className="flex flex-col gap-3">
        <Field label="Area: " value={currentAsset.area} />
        <Field label="Location From: " value={item?.pending?.location ?? ""} />
        <Field
          label="Date of Disposal "
          value={item?.pending?.expectedDisposalDate ?? ""}
        />
        <Field label="Issue Reason: " value={currentAsset.assetIssueReason} />
        <Field label="Issue Details: " value={currentAsset.assetIssueDetails} />
      </div>

      {/* ── Description box ── */}
      {/* {item?.pending?.description && (
        <div className="flex flex-col gap-1.5">
          <span className="text-xs text-gray-400 dark:text-gray-500">
            Description
          </span>
          <div className="bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700 rounded-md px-3 py-2.5 text-sm leading-relaxed text-gray-800 dark:text-gray-200">
            {item?.pending.description}
          </div>
        </div>
      )} */}

      {/* ── Comments ── */}
      {item?.pending?.disposalReason && (
        <div className="flex flex-col gap-1.5">
          <span className="text-xs text-gray-400 dark:text-gray-500">
            Reason
          </span>
          <div className="bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700 rounded-md px-3 py-2.5">
            <p className="text-sm text-gray-800 dark:text-gray-200">
              {item?.pending?.disposalReason}
            </p>
          </div>
        </div>
      )}

      {/* ── Actions ── */}
      <div className="mt-auto pt-4 flex flex-col gap-6">
        <p className="text-xs text-center text-gray-400 dark:text-gray-500">
          Review all details before approving this request.
        </p>
        <div className={cn(sharedStyles.btnParent, "md:w-full")}>
          <button
            type="button"
            onClick={handleReject}
            className={cn(
              sharedStyles.btnCancel,
              sharedStyles.btn,
              "flex items-center justify-center gap-4",
            )}
          >
            <X className="w-6 h-6" />
            <span className="text-md">Reject</span>
          </button>
          <button
            type="submit"
            disabled={isApproving}
            className={cn(
              sharedStyles.btnApprove,
              sharedStyles.btn,
              "flex items-center justify-center gap-4",
            )}
            onClick={handleApprove}
          >
            {isApproving ? (
              <Spinner className="w-6 h-6" />
            ) : (
              <Check className="w-6 h-6" />
            )}
            <span className="text-md">Approve</span>
          </button>
        </div>
      </div>
    </div>
  );
}

export default DisposalRequestApproval;

/* -------------------------------------------------------------------------- */
/*                       OLD VIEW - SINGLE ASSET                              */
/* -------------------------------------------------------------------------- */

// //$ This component display detailed information of the the maintenance request created and actioned data.

// import Separator from "@/components/dashboardSidebar/Seperator";
// import type { TransferWorkflowResponse } from "@/schemas";
// // import { useNavigate } from "react-router-dom";
// import useGlobalContext from "@/context/useGlobalContext";
// import { useById } from "@/utils/api";
// import { Error } from "../features/Error";
// import { PageLoadingSpinner } from "../features/PageLoadingSpinner";
// import { toast } from "sonner";

// // icons
// import { X, Check } from "lucide-react";
// import { Badge } from "../features/Badge";
// import { badgeStyles } from "@/styles/badgeStyles";
// import { sharedStyles } from "@/styles/shared";
// import { cn } from "@/lib/utils";
// import Avatar from "../header/Avatar";
// import axios from "axios";
// import { Spinner } from "../ui/spinner";
// import { useApproveRequest } from "@/hooks/useApproveRequest";

// function Field({ label, value }: { label: string; value?: string | null }) {
//   if (!value) return null;
//   return (
//     <div className="grid grid-cols-[120px_1fr] gap-2 items-start text-sm">
//       <span className="text-xs text-gray-500 dark:text-gray-400 pt-0.5">
//         {label}
//       </span>
//       <span className="text-gray-900 dark:text-gray-100 capitalize font-medium">
//         {value}
//       </span>
//     </div>
//   );
// }

// function TransferRequestApproval() {
//   const {
//     selectedRowId,
//     showRejectRequestDialogGeneric,
//     setShowRejectRequestDialogGeneric,
//     setRejectConfig,
//   } = useGlobalContext();

//   console.log("genericModal:", showRejectRequestDialogGeneric);

//   /* -------------------------------------------------------------------------- */
//   /*                          Item Data                                         */
//   /* -------------------------------------------------------------------------- */

//   const { data: item, isPending } = useById<TransferWorkflowResponse>({
//     id: selectedRowId ?? "",
//     queryKey: ["transfers", "pending-approval-item"],
//     resourcePath: "api/transfers",
//     params: {
//       status: "pending",
//     },
//   });

//   /* -------------------------------------------------------------------------- */
//   /*                          Submit Data Hook                                  */
//   /* -------------------------------------------------------------------------- */

//   const { submit: approveRequest, isPending: isApproving } = useApproveRequest({
//     id: selectedRowId ?? "",
//     resourcePath: "api/transfers",
//     queryKey: ["transfers", "action: approve-item"],
//     successMessage: `The Request for asset ${item?.assetID} was Successfully Approved!!!`,
//     errorMessage: "Could not approve the asset transfer. Please try again.",
//     redirectPath: "transfers/requests",
//   });

//   /* -------------------------------------------------------------------------- */
//   /*                          Handle Approve                                    */
//   /* -------------------------------------------------------------------------- */
//   const handleApprove = async () => {
//     try {
//       await approveRequest({});
//     } catch (error) {
//       if (axios.isAxiosError<{ message: string }>(error)) {
//         toast.error(error?.response?.data?.message);
//       } else {
//         toast.error("Failed to assign item");
//       }
//     }
//   };

//   if (isPending) {
//     return <PageLoadingSpinner />;
//   }

//   // fallback UI if timeout reached
//   if (!item) {
//     return <Error />;
//   }

//   /* -------------------------------------------------------------------------- */
//   /*                          Handle Reject                                     */
//   /* -------------------------------------------------------------------------- */

//   const handleReject = () => {
//     setRejectConfig({
//       title: "Reject Transfer",
//       message: "Are you sure you want to reject this transfer?",
//       resourcePath: "api/transfers",
//       redirectPath: "transfers/requests",
//       queryKey: ["transfers", "action: reject"],
//       successMessage: "The Request was Successfully Rejected.",
//       errorMessage: "Could not reject the asset transfer. Please try again.",
//     });
//     setShowRejectRequestDialogGeneric(true);
//   };

//   return (
//     <div className="flex flex-col gap-4 text-font dark:text-gray-100 rounded-md p-4 dark:border-gray-700/50 h-full">
//       {/* // $ ── Header ── */}
//       <div className="flex flex-col gap-2">
//         <p className="text-[11px] uppercase tracking-widest text-gray-400 dark:text-gray-500 font-medium select-none">
//           Transfer No ·{""}
//         </p>
//         {/* Here i'm only showing the first equipment, will change when adding the list */}
//         <h1 className="text-lg md:text-xl font-semibold capitalize leading-tight">
//           {item?.pending?.assets[0].equipment}
//         </h1>
//         <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">
//           Asset ID: {item?.assetID}
//         </p>
//       </div>

//       {/* ── Status badges ── */}
//       <div className="flex flex-wrap gap-2 text-cxs">
//         <Badge
//           value={`${item.status ?? "medium"}`}
//           styleMap={badgeStyles.families.transfer_status}
//           className="capitalize"
//         />
//       </div>
//       <Separator width="100%" className="mt-2 mb-4" />

//       {/* ── Requester row ── */}
//       {item.pending?.requested_by && (
//         <div className="flex items-center gap-3">
//           <Avatar name={item.pending?.requested_by} isFullName={true} />
//           <div className="flex flex-col leading-snug">
//             <span className="text-sm font-medium capitalize">
//               {item?.pending.requested_by}
//             </span>
//             <span className="text-xs text-gray-500 dark:text-gray-400">
//               {`Requested · ${item.transferCreated}`}
//             </span>
//           </div>
//         </div>
//       )}

//       {/* ── Structured fields ── */}
//       <div className="flex flex-col gap-3">
//         <Field
//           label="Location From: "
//           value={item?.pending?.locationFrom ?? ""}
//         />
//         <Field label="Location To: " value={item?.pending?.locationTo ?? ""} />
//         <Field
//           label="Date of Transfer "
//           value={item?.pending?.expectedDate ?? ""}
//         />
//       </div>

//       {/* ── Description box ── */}
//       {item?.pending?.description && (
//         <div className="flex flex-col gap-1.5">
//           <span className="text-xs text-gray-400 dark:text-gray-500">
//             Description
//           </span>
//           <div className="bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700 rounded-md px-3 py-2.5 text-sm leading-relaxed text-gray-800 dark:text-gray-200">
//             {item?.pending.description}
//           </div>
//         </div>
//       )}

//       {/* ── Comments ── */}
//       {item?.pending?.transferReason && (
//         <div className="flex flex-col gap-1.5">
//           <span className="text-xs text-gray-400 dark:text-gray-500">
//             Reason
//           </span>
//           <div className="bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700 rounded-md px-3 py-2.5">
//             {/* <p className="text-xs text-gray-400 dark:text-gray-500 mb-1 capitalize">
//               {item.requested_by}
//             </p> */}
//             <p className="text-sm text-gray-800 dark:text-gray-200">
//               {item?.pending?.transferReason}
//             </p>
//           </div>
//         </div>
//       )}

//       {/* ── Actions ── */}
//       <div className="mt-auto pt-4 flex flex-col gap-6">
//         <p className="text-xs text-center text-gray-400 dark:text-gray-500">
//           Review all details before approving this request.
//         </p>
//         <div className={cn(sharedStyles.btnParent, "md:w-full")}>
//           <button
//             type="button"
//             onClick={handleReject}
//             className={cn(
//               sharedStyles.btnCancel,
//               sharedStyles.btn,
//               "flex items-center justify-center gap-4",
//             )}
//           >
//             <X className="w-6 h-6" />
//             <span className="text-md">Reject</span>
//           </button>
//           <button
//             type="submit"
//             disabled={isApproving}
//             // variant="submit"
//             // size="lg"
//             className={cn(
//               sharedStyles.btnApprove,
//               sharedStyles.btn,
//               "flex items-center justify-center gap-4",
//             )}
//             onClick={handleApprove}
//           >
//             {isApproving ? (
//               <Spinner className="w-6 h-6" />
//             ) : (
//               <Check className="w-6 h-6" />
//             )}
//             <span className="text-md">Approve</span>
//           </button>
//         </div>
//       </div>
//     </div>
//   );
// }

// export default TransferRequestApproval;
