...
const CreateDisposalCompletedForm = () => {
  ...
  const { setSuccessConfig, setShowSuccess, setErrorConfig, setShowError } =
    useGlobalContext();
  const { data: disposal, isPending: isLoading, isError, refetch } =
  ...
  const form = useForm<DisposalCompletionFormValues>({
    resolver: zodResolver(completionSchema) as Resolver<DisposalCompletionFormValues>,
    defaultValues: {
      disposalMethod: "",
      disposalLocation: "",
      disposalCost: null,
  ...
  >({
    resourcePath: `api/disposals/${encodeURIComponent(id ?? "")}/dispose`,
    queryKey: ["disposals"],
    extractFiles: (values) => ({
    ...
    onError: () => {
      setErrorConfig({
        ...
        message: "Could not complete the disposal or upload its attachments. Check the disposal status before trying again.",
      });
      setShowError(true);
      void queryClient.invalidateQueries({ queryKey: ["disposal"] });
    },
  ...
  const fields: DynamicFormField<DisposalCompletionFormValues>[] = [
    {
      ...
      placeholder: "e.g. Recycled, sold, donated",
      required: true,
    },
    {
      fieldType: "input",
      name: "disposalLocation",
      label: "Disposal location (optional)",
      placeholder: "Where the assets were disposed of",
    },
    {
      ...
      render: ({ field, fieldState }) => (
        <div className="flex flex-col gap-2">
          <label htmlFor="disposalCost" className="text-sm">Disposal cost (R, optional)</label>
          <input
            id="disposalCost"
            name={field.name}
            ref={field.ref}
            ...
            className={`${sharedStyles.formInput} ${sharedStyles.formInputDefault}`}
            aria-invalid={!!fieldState.error}
            aria-describedby={fieldState.error ? "disposalCost-error" : undefined}
          />
          {fieldState.error && <p id="disposalCost-error" className="text-xs text-red-500">{fieldState.error.message}</p>}
        </div>
      ),
    },
    {
    ...
    },
    {
      fieldType: "file",
      name: "disposalImages",
      label: "Disposal photos (optional)",
      multiple: true,
      className: "col-span-full",
    },
    {
  ...
  if (isError) return (
    <div role="alert">
      ...
      <button type="button" onClick={() => void refetch()} className="text-blue-500">Try again</button>
    </div>
  );
  if (!disposal) return <p role="alert">Disposal request not found.</p>;
  if (disposal.status !== "approved") return (
    <div className="space-y-3" role="status">
      <p>Only approved requests can be completed. This disposal is {disposal.status}.</p>
      <button type="button" onClick={() => navigate("/disposals/requests")} className="text-blue-500">Back to disposals</button>
    </div>
  );
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-medium">
              {disposal.assets.length} {disposal.assets.length === 1 ? "asset" : "assets"} approved for disposal
            </span>
            <span className="mt-0.5 block text-xs capitalize text-gray-500 dark:text-gray-400">
              {disposal.pending.location} · {disposal.pending.disposalReason.replaceAll("-", " ")}
            </span>
          </span>
          <span className="hidden text-xs text-gray-500 sm:inline dark:text-gray-400">Request details</span>
          <ChevronDown className={`size-4 shrink-0 text-gray-400 transition-transform ${requestOpen ? "rotate-180" : ""}`} aria-hidden="true" />
        </button>
              ...
              <div className="space-y-3 border-t border-gray-200 px-4 py-3 dark:border-gray-700">
                <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                  {disposal.assets.map((asset, index) => (
                    <li key={asset.assetIndex ?? index} className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 py-2 text-sm">
                      <span className="capitalize">{asset.equipment}</span>
                      <span className="text-xs text-gray-500 dark:text-gray-400">{asset.assetID || "Unidentified asset"}</span>
                    </li>
                  ))}
                </ul>
                <p className="text-xs text-gray-500 dark:text-gray-400">Completion applies to all assets in this request.</p>
                <p className="break-all text-xs text-gray-400 dark:text-gray-500">Request ID: {disposal.id}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      <fieldset disabled={isPending || form.formState.isSubmitting} className="min-w-0">
